...
export const cloudStorageConfig = {
  googleDrive: {
    ...
    redirectUri: (typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_FRONTEND_ORIGIN || 'https://ezsignature.org')) + '/auth/google-drive/callback'
  },
  
  dropbox: {
    appKey: 'YOUR_DROPBOX_APP_KEY',
    appSecret: 'YOUR_DROPBOX_APP_SECRET',
    redirectUri: (typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_FRONTEND_ORIGIN || 'https://ezsignature.org')) + '/auth/dropbox/callback'
  },
  
  box: {
    clientId: 'YOUR_BOX_CLIENT_ID',
    clientSecret: 'YOUR_BOX_CLIENT_SECRET',
    redirectUri: (typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_FRONTEND_ORIGIN || 'https://ezsignature.org')) + '/auth/box/callback'
  },
  
  oneDrive: {
    clientId: 'YOUR_ONEDRIVE_CLIENT_ID',
    clientSecret: 'YOUR_ONEDRIVE_CLIENT_SECRET',
    redirectUri: (typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_FRONTEND_ORIGIN || 'https://ezsignature.org')) + '/auth/onedrive/callback',
    scopes: ['Files.Read']
  }
};
...
export const getCloudStorageConfig = () => ({
  googleDrive: {
    ...
    apiKey: process.env.NEXT_PUBLIC_GOOGLE_DRIVE_API_KEY || cloudStorageConfig.googleDrive.apiKey,
  ...
  dropbox: {
    appKey: process.env.NEXT_PUBLIC_DROPBOX_CLIENT_ID || cloudStorageConfig.dropbox.appKey,
    appSecret: process.env.DROPBOX_CLIENT_SECRET || cloudStorageConfig.dropbox.appSecret,
