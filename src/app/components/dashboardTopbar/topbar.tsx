...
const Topbar:FC<TopbarProps> = ({children,title,buttonText,secondText,hideRightContent = true,isBellIcon,isCaretIcon})=>{
    ...
    <Sidebar open = {showSideBar} onToggle = {handleDrawerToggle}>
       <Box bgcolor={"#fff"} position={"fixed"} top={0} right={0}  ml= {{ md: `250px` }} left={0} borderBottom={"1px solid #CED4DA"} maxWidth={{md:"calc(100% - 250px)"}} component={"div"}>  
        ...
        <Box px={3} display={"flex"} justifyContent={"space-between"} alignItems={"center"}  height={66}>
          ...
          {buttonText && <Button  color="#fff" fontWeight="500" fontSize={16} borderRadius={20} backgroundColor="var(--secondary-color)" height={"35px"} width={"fit-content"} style={{padding:"0 25px", display:"flex",alignItems:"center"}}>{buttonText} {isCaretIcon && <KeyboardArrowDownIcon/>}</Button> }
          {secondText && <Button  color="#fff" fontWeight="500" fontSize={16} borderRadius={20} backgroundColor="var(--secondary-color)" height={"35px"} width={"fit-content"} style={{padding:"0 25px", display:"flex",alignItems:"center"}}>{secondText} {isCaretIcon && <KeyboardArrowDownIcon/>}</Button> }
