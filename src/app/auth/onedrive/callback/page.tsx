    ...
    if (code) {
      (async () => {
        try {
          ...
          const resp = await fetch(`${apiBase.replace(/\/$/, '')}/api/cloud-storage/providers/onedrive/callback`, {
            method: 'POST',
            headers: {
          ...
          if (resp.ok && json?.success) {
            try { localStorage.setItem('cloud_connected_onedrive', 'true'); } catch {}
          } else {
            console.error('OneDrive callback failed', json);
          }
        } catch (e) {
