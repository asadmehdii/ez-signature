...
const handleSendDocument = async () => {
  if (!storedFile) {
    showNotification('error', 'No File', 'No file to send!');
    return;
  }

  try {
    // Get the file URL from documentData instead of storedFile
    const fileUrl = documentData?.fileUrl || storedFile.dataUrl;
  ...
  switch (type) {
    ...
    formData.append('file', pdfBlob, `final_document_${Date.now()}.pdf`);
    ...
    console.log("Uploading final document to backend...");
    
    const token = localStorage.getItem('token');
    const apiBaseForUpload = process.env.NEXT_PUBLIC_API_BASE || (typeof window !== 'undefined' ? `${window.location.origin}/api` : '');
    const uploadResponse = await fetch(`${apiBaseForUpload}/document/upload-final`, {
      method: "POST",
      headers: {
    ...
    console.log("Backend upload success:", uploadResult);

    const fileDownloadUrl = uploadResult.fileUrl;

    if (!fileDownloadUrl) {
      throw new Error("Failed to get file URL from backend");
    }

    const allRecipientEmails = recipients.map(r => r.email);
    await sendEmail(allRecipientEmails, fileDownloadUrl, 'Final Document');

    showNotification('success', 'Document Sent', 'Document sent successfully!');

  } catch (error) {
    console.error("Error sending document:", error);
    showNotification('error', 'Send Failed', `Error sending document: ${error.message}`);
  }
};
...
async function sendEmail(recipients, fileUrl, fileName) {
  const token = localStorage.getItem('token');
  const apiBase = process.env.NEXT_PUBLIC_API_BASE || (typeof window !== 'undefined' ? `${window.location.origin}/api` : '');
  const emailApiUrl = `${apiBase}/email/send`;
